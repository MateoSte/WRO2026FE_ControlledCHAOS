...
def detekcija():
    while True:
        data=call('blocks')
        print(data)
        if data[0]==1:  #data[0]-> ID objekta prepoznatog po boji
            hub.display.char('R')
        elif data[0]==2:
            hub.display.char('G')
...
while not connected and attempts < 20:
    try:
        wait(2000)  # Daj ESP32 vremena za boot
        init('A')
        connected = True
...
if not connected:
    print("Failed to connect to ESP32 after 5 attempts")
    raise Exception("Could not establish PUP connection")
...
def obilaznica():
    global prepreka
    prepoznajBoju()
    if(prepreka[0] == 1): #zelena
        zaobilazak_lijevo(prepreka[1])
    else: #crvena
        zaobilazak_desno(prepreka[1])
    prepreka = [0,0]
